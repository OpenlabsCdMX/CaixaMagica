...
class Request {
    ...
    private function parseIncomingParams() {
        ...
        if (filter_input(INPUT_SERVER,'QUERY_STRING') != null) {
            parse_str(filter_input(INPUT_SERVER,'QUERY_STRING'), $parameters);
        }

        $body = file_get_contents("php://input");
        $content_type = ( filter_input(INPUT_SERVER,"CONTENT_TYPE") != null) ?
                filter_input(INPUT_SERVER,'CONTENT_TYPE') : false;
   ...
   public static function response($msg,$args,$error = 0,$returnJson = 0){
       $response = [
               "error"=>$error,
                "msg"=>$msg
            ];
            ...
            $response = ($returnJson)? json_encode($response,true) : $response;
        return $response;
   }
}
